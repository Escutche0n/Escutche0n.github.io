...
各个相机品牌之间 RAW 文件命名方式不同，如松下的 `RW2`，佳能的 `CR2`， 尼康的 `DNG`，以及索尼的`ARW`等。大多数厂牌的 RAW 可以按文件夹直接导入 Lightroom 处理图像。通常意义上同一张照片的 RAW 图像会比传统的 jpg 有着更高的宽容度，意味着其保留着更多画面细节的同时带来了更广阔的后期空间。
